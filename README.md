...
Phase 49 adds an explicit evidence freshness policy and stale review queue to `POST /api/analyze-stock`. `evidence_freshness_policy` documents non-scoring freshness windows for manual evidence, provider caches, market data, filings, Form 4, 13F, and macro sources. `stale_review_queue` lists stale manual evidence, due-for-review evidence, missing-source-date evidence, and stale live/cached/derived source-status items that should be refreshed, reviewed, or archived before increasing validation confidence. The queue is explanation/workflow metadata only and does not change final score or verdict.

Phase 50 adds yfinance insider-ownership auto evidence for Jane C2 and refines C3 short-interest thresholds. Analyze-stock can now partially cover C2 `founder_ownership` from yfinance `heldPercentInsiders`, while keeping `founder_is_ceo` and founder quality submetrics manual. C3 `short_interest_proxy` now uses stricter high/moderate/low skepticism thresholds from `shortRatio` and `shortPercentOfFloat`, and very low short-interest data no longer fills coverage by itself. These Coverage Matrix items are non-scoring auto-derived financial proxies with explicit verification limitations.

Phase 51 adds ADR / foreign-filer coverage diagnostics and a manual research path. `foreign_filer_coverage_diagnostics` separates structural SEC Companyfacts, SEC Form 4, 13F, and FMP transcript coverage gaps from company-specific weakness, marks them as non-scoring, and adds ADR-aware manual checks to affected Jane Coverage Matrix rows.

Phase 52 adds ADR Manual Evidence Intake and Filing Reference Workflow. Request-scoped and saved manual evidence can preserve `adr_evidence_type`, `document_title`, `document_date`, `filing_period`, `quoted_text`, `local_market`, `local_ticker`, and `translation_note`; filing-backed ADR evidence can map explicitly supplied `criterion_id` / `submetric` values into Jane Coverage Matrix completeness while remaining user-provided, manually reviewed, non-scoring, and not investment advice.

Phase 53 refines C18 USPTO activation and ADR grade explanations without changing scoring. Disabled USPTO PatentsView now appears in C18 Coverage Matrix limitations / `next_manual_check` as `USE_LIVE_USPTO_PATENTS_DATA=true` activation guidance. ADR / foreign-filer C3 short-interest gaps from unavailable yfinance `shortRatio` / `shortPercentOfFloat` are explained as source coverage limitations, and ADR source-quality Grade D copy says the grade reflects data-source coverage / data-structure limits rather than company-quality weakness.

Phase 54 integrates the ADR Manual Evidence Library UX with the local review queue. Saved manual evidence now uses ADR `document_date` as a `source_date` fallback for freshness review, `GET /api/manual-evidence/dashboard` queue items surface ADR metadata (`adr_evidence_type`, `document_title`, `document_date`, `filing_period`, `local_market`, `local_ticker`) plus `adr_review_label`, `adr_review_guidance`, `affects_score=false`, and `not_investment_advice=true`, and the Evidence Library form exposes ADR helper fields for filing-backed manual review without fetching URLs or changing scoring.

Phase 55 expands Coverage Matrix auto-evidence bridging without changing final scoring or verdicts. USPTO PatentsView C18 `patent_count` is enabled by default as a no-key provider while still honoring `USE_LIVE_USPTO_PATENTS_DATA=false`; existing SEC 13F target-match context can partially cover C19 `institutional_support` / `fund_support` as delayed, filing-backed, manual-review evidence. Phase 56 tightens the user-supplied theme boundary: `research_context.theme` now appears in `theme_validation_context` as a validation target only, with `theme_discovery_enabled=false`, `ranking_or_scoring_policy="not_ranked_or_scored"`, `affects_score=false`, and C11 `jane_theme_alignment` still requiring explicit manual evidence for revenue exposure, industry CAGR, policy support, and capital inflow.

Phase 60A hardens Jane Coverage Matrix auto-evidence boundaries without changing final scoring or verdicts. C3 yfinance `shortRatio` / `shortPercentOfFloat` evidence now surfaces as explicit short-interest proxy coverage or a readable manual gap; C19 now warns when a `SEC_13F_TARGET_MANAGERS` deployment override drops default core managers such as Vanguard, BlackRock, or State Street; and C11 continues to reject theme text alone while allowing explicitly supplied criterion/submetric evidence to cover user-provided mega-trend submetrics.
...
- Phase 42: FMP stable Financial Statements + TTM Ratios proxy for ADR / SEC Companyfacts gaps
...
- Phase 49: Evidence Freshness Policy and Stale Review Queue for analyze-stock review workflow
- Phase 50: Yfinance C2 insider-ownership proxy and stricter C3 skepticism threshold refinement
- Phase 51: ADR / foreign-filer coverage diagnostics and manual research path
...
- Phase 55: Coverage Matrix auto-evidence expansion for default USPTO C18, SEC 13F C19, and user-theme C11 context
...
| SEC_13F_TARGET_MANAGERS | 0001067983,0000102909,0001364742,0000093751,0001214717 | SEC EDGAR 13F | optional comma-separated manager names or CIKs; default universe is Berkshire, Vanguard, BlackRock, State Street, and Geode. Deployment overrides replace, not merge with, the default list; analyze-stock warns when the override drops default core managers. |
...
| USE_LIVE_USPTO_PATENTS_DATA | true | USPTO PatentsView C18 evidence | no API key required; set false to disable |
...
- Deployment overrides replace the default manager universe rather than merging with it. If an override omits default core managers, analyze-stock adds a non-scoring warning to `institutional_13f.limitations` and C19 Coverage Matrix limitations so `no_reported_13f_position_observed` regressions can be traced to configuration instead of company quality.
...
Phase 59 adds `platform_business_quality_card` (`phase59_platform_business_quality_card_v1`) to `POST /api/analyze-stock` and the Stock Research UI. It organizes platform-business quality checks for `gmv_growth`, `take_rate`, `net_dollar_retention`, `burn_rate`, `runway`, `marketplace_liquidity`, `network_effect`, `ltv_cac`, and `contribution_margin_operating_leverage`. The card computes only rough public-data proxies that are already available, such as negative free-cash-flow burn/runway and operating-margin/free-cash-flow-margin operating leverage. GMV, take rate, NDR, marketplace liquidity, network effect, and LTV/CAC remain manual or disclosed evidence unless supplied with source context. Phase 59 does not change final score, research verdict, confidence gates, or Jane criteria scoring.
