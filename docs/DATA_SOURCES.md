...
Phase 26 adds validation-quality explanations without adding providers or fetch behavior. SEC/yfinance discrepancy explanations help reviewers understand provider normalization, filing concept coverage, and period-alignment limits. Legacy mock leadership remains visible only as deprecated compatibility context. Smart-money source-quality breakdowns disclose delayed quarterly 13F, Form 4 fallback/cached limits, and mock options context. Valuation-risk explanations are research context only, and prioritized manual checks are validation tasks.
