...
- `foreign_filer_coverage_diagnostics`: non-scoring Phase 51 ADR / foreign-filer diagnostics that separate structural SEC Companyfacts, SEC Form 4, 13F, FMP transcript, yfinance short-interest, and local-filing coverage gaps from company-specific weakness and provide manual research checks.
...
- `data_quality_summary.foreign_filer_context` explains structural ADR / foreign-filer coverage limitations, including SEC Companyfacts, 13F candidate-specific gaps, yfinance short-interest gaps, and source-quality data-structure limits, so normal non-US coverage limits are not presented as company-specific weakness.
- Phase 53 refines this wording without changing schema or scoring: unavailable ADR `shortRatio` / `shortPercentOfFloat` is a C3 source coverage limitation, `source_quality_grade="D"` can describe data-source coverage / data-structure limits rather than company quality, and disabled USPTO PatentsView is exposed as C18 activation guidance (`USE_LIVE_USPTO_PATENTS_DATA=true`).
...
- Phase 54 extends the saved Manual Evidence Library UX and review queue for the same ADR metadata. `POST /api/manual-evidence` and `PATCH /api/manual-evidence/{evidence_id}` accept the ADR filing fields, saved ADR evidence uses `document_date` as a `source_date` fallback for freshness review, and `GET /api/manual-evidence/dashboard` queue items surface ADR metadata plus `adr_review_label`, `adr_review_guidance`, `affects_score=false`, and `not_investment_advice=true` without fetching URLs, verifying claims, or changing score/verdict semantics.
- Phase 55 expands Coverage Matrix auto-evidence using existing provider/context outputs only: no-key USPTO PatentsView C18 `patent_count` is enabled by default but still opt-out via `USE_LIVE_USPTO_PATENTS_DATA=false`; existing SEC 13F target-match output can partially cover C19 `institutional_support` / `fund_support` as delayed filing-backed context. Phase 56 supersedes the earlier C11 theme-text auto-coverage boundary: explicit `research_context.theme` is a validation target only and does not partially cover C11 without separate evidence. These rows are manual-review, non-scoring, and do not change final score, verdict, or investment-advice boundaries.
...
- Phase 50 adds non-scoring C2 (`founder_ownership`) coverage from yfinance `heldPercentInsiders` / normalized insider ownership, while keeping `founder_is_ceo`, vision consistency, milestone execution, and crisis execution manual. It also refines C3 thresholds so very low short interest does not fill `short_interest_proxy`; high/moderate/low labels come from `shortRatio` and `shortPercentOfFloat`. Phase 53 clarifies that ADR / foreign-filer missing `shortRatio` / `shortPercentOfFloat` is a source coverage limitation, not company-quality weakness.
- Phase 47 adds non-scoring provider-backed C18 (`patent_count`) coverage from USPTO PatentsView `patent_ip_evidence`. It remains an auto-derived patent-count proxy and does not prove patent relevance, licensing value, or defensibility. Phase 53 surfaces disabled-provider activation guidance in C18 Coverage Matrix limitations / `next_manual_check` when `USE_LIVE_USPTO_PATENTS_DATA=false`.
- Phase 55 makes the no-key USPTO C18 provider default-on unless explicitly disabled, maps existing SEC 13F target-match context into C19 (`institutional_support`, `fund_support`) with `financial_proxy_source="sec_13f"`, and maps explicit user theme context into C11 `jane_theme_alignment` with `source_quality="user_context"`. All three are Coverage Matrix completeness context only and require manual review.
...
- Phase 53 ensures disabled PatentsView is also visible in C18 Coverage Matrix limitations / `next_manual_check` as activation guidance; disabled C18 provider state remains non-scoring and should not be read as weak company IP by itself.
...
- Phase 69 adds Manual Evidence Quality Loop metadata to saved evidence: `linked_gap_id`, `linked_criterion_id`, `linked_submetrics`, `resolution_status` (`unresolved`, `candidate_evidence_present`, `pending_review`, `incomplete`, `stale`, `resolved_for_review`), `missing_required_fields`, `review_state`, `freshness_state`, and `evidence_quality_note`. This metadata can appear in analyze-stock `qualitative_evidence_assessment.evidence_items[]`, `evidence_gap_inbox.items[].manual_evidence_resolution`, and `jane_criteria_coverage.criteria[].manual_evidence_resolution`; `manual_evidence_resolution` uses the `ManualEvidenceResolution` contract shape with linked evidence count/ids, review/freshness state, missing-field guidance, and non-scoring flags. It is non-scoring and does not fetch or validate source URLs.
...
- Phase 54 exposes the same ADR filing-reference fields in the Evidence Library form and PATCH contract. For saved ADR records, `document_date` is copied into `source_date` when `source_date` is omitted so dashboard freshness and review-queue logic can use the filing date. If both `document_date` and `source_date` are missing, dashboard `review_queue` uses `review_due_reason="source_date_missing"` and `adr_review_label="ADR filing metadata incomplete"`.
...
Phase 54 queue items may include ADR review fields when saved manual evidence has `adr_evidence_type`: `adr_evidence_type`, `document_title`, `document_date`, `filing_period`, `local_market`, `local_ticker`, `adr_review_label`, `adr_review_guidance`, `affects_score=false`, and `not_investment_advice=true`. `adr_review_guidance` reminds the user that local filing evidence is user-provided, not independently verified, review-queue workflow metadata only, and does not change scoring or verdict semantics.
...
### Phase 58 `company_event_signal_breakdown`

`POST /api/analyze-stock` returns `company_event_signal_breakdown` with version `phase58_company_event_signal_breakdown_v1`. Fields include `summary`, `event_signal_count`, `event_signals`, `insider_summary`, `institutional_summary`, `options_summary`, `lockup_summary`, `manual_review_required`, `manual_checks`, `limitations`, `affects_score`, `final_score_unchanged`, and `not_investment_advice`. Each event signal carries `name`, `category`, `label`, `observed_value`, `source_quality`, `source_date`, `interpretation`, `manual_check`, `limitations`, `affects_score`, and `is_real_time_signal`. Covered MVP signals are `form4_insider_accumulation_disposition`, `systematic_insider_plan_risk`, `delayed_13f_institutional_positioning`, `options_attention_abnormality`, and `ipo_lockup_expiration`. Phase 58 is research context only, not a trading signal, and does not change final score, research verdict, confidence gates, or scoring weights. Lock-up data is not fetched automatically; `lockup_summary` remains manual verification context.

### Phase 59 `platform_business_quality_card`

`POST /api/analyze-stock` returns `platform_business_quality_card` with version `phase59_platform_business_quality_card_v1`. Fields include `summary`, `platform_metric_count`, `computed_metric_names`, `manual_evidence_metric_names`, `manual_or_disclosed_metric_names`, `metrics`, `manual_review_required`, `manual_checks`, `limitations`, `affects_score`, `final_score_unchanged`, and `not_investment_advice`. Each metric carries `name`, `label`, `category`, `status`, `observed_value`, `source_quality`, `source_date`, `interpretation`, `manual_check`, `limitations`, `requires_manual_evidence`, and `affects_score`. MVP metric names are `gmv_growth`, `take_rate`, `net_dollar_retention`, `burn_rate`, `runway`, `marketplace_liquidity`, `network_effect`, `ltv_cac`, and `contribution_margin_operating_leverage`. GMV, take rate, NDR, marketplace liquidity, network effect, and LTV/CAC are manual or disclosed evidence unless supplied with source context; burn/runway and operating leverage are computed only from already available public financial proxies. Phase 59 does not change final score, research verdict, confidence gates, or scoring weights.
