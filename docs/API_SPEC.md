...
- `jane_criteria_coverage`: non-scoring coverage matrix across the canonical Jane 20 criteria, accepted evidence items, SEC Companyfacts financial proxies where available, C2/C3/C5 auto-derived yfinance/financial proxies, C11 user-theme `jane_theme_alignment`, C18 PatentsView patent-count proxy, C19 SEC 13F institutional-support context, covered and missing submetrics, and next manual checks. Phase 53 adds explicit C18 activation guidance when USPTO PatentsView is disabled; Phase 55 enables no-key USPTO by default and bridges existing 13F/theme context without changing final scoring.
...
- Phase 55 expands Coverage Matrix auto-evidence using existing provider/context outputs only: no-key USPTO PatentsView C18 `patent_count` is enabled by default but still opt-out via `USE_LIVE_USPTO_PATENTS_DATA=false`; existing SEC 13F target-match output can partially cover C19 `institutional_support` / `fund_support` as delayed filing-backed context; explicit `research_context.theme` can partially cover C11 `jane_theme_alignment` as user-context evidence. These rows are manual-review, non-scoring, and do not change final score, verdict, or investment-advice boundaries.
